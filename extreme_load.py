...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run extreme-load inference from checkpoint."
    )
    parser.add_argument(
        "--dataset-dir",
        type=str,
    ...
    data_path = dataset_dir / "extreme_forecast_task_L14d_2022-04-15_to_2022-04-29.csv"
    if not data_path.exists():
        raise FileNotFoundError(f"Dataset not found: {data_path}")

    model_dir = work_dir / "extreme"
    ...
    train, val = series.split_after(pd.Timestamp("2022/04/29 0:00:00"))

    scaler = Scaler()
    train_scaled = scaler.fit_transform(train)

    load_model = TFTModel.load_from_checkpoint("extreme", best=True, work_dir=str(work_dir))
    ...
    ax.yaxis.set_major_locator(ticker.MultipleLocator(1500))
    ...
    fig.savefig(output_dir / "extreme_day_forecast.pdf", bbox_inches="tight", pad_inches=SAVEFIG_PAD_INCHES)
    fig.savefig(output_dir / "extreme_day_forecast.png", dpi=300, bbox_inches="tight", pad_inches=SAVEFIG_PAD_INCHES)
    if args.no_show:
        plt.close(fig)
    else:
        plt.show()
    ...
    qdf.to_csv(output_dir / "pred_quantiles_extreme.csv")
    for ci in [0.9, 0.8, 0.7, 0.6, 0.5]:
        low = (1 - ci) / 2
        high = 1 - low
        df_ci = pd.concat([pred.quantile_df(low), pred.quantile_df(high)], axis=1)
        df_ci.to_csv(output_dir / f"pred_ci_{int(ci * 100)}_extreme.csv")
